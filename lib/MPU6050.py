...
class MPU6050():
    ...
    def __init__(self, side=1, disable_interrupts=False, importI2C):

        # create i2c object
        self._timeout = 10
        self.disable_interrupts = False
        self._mpu_i2c = esp.I2C(side, importI2C)
        # self._mpu_i2c = esp.I2C(side, esp.I2C.MASTER)
    ...
    def _read(self, count, memaddr, devaddr):
        ...
        if self.disable_interrupts:
            irq_state = esp.disable_irq()
        result = self._mpu_i2c.mem_read(count,
                                        devaddr,
                                        memaddr,
                                        timeout=self._timeout)
        esp.enable_irq(irq_state)
        return result
